...
func _ready():
	...
	var pos = Rooms[0]
	var local_pos = self.map_to_local(Vector3i(pos.x, 0, pos.y))
...
func SpawnBorder():
	for x in get_parent().DungeonSize.x + 10:
		for y in get_parent().DungeonSize.y + 10:
			set_cell_item(Vector3i(x-5, 0, y-5), 1, 0)
			...
			for y in w:
				...
				if x == 0 or y == 0 or x == h-1 or y == w-1 or pos.x == 3 or pos.x == $"../".DungeonSize.x-3 or pos.y == 3 or pos.y == $"../".DungeonSize.y-3:
					if IsValid(pos.x, pos.y):
						if get_cell_item(Vector3i(pos.x, 0, pos.y)) == 1:
							set_cell_item(Vector3i(pos.x, 0, pos.y), 0, 0)
						if get_cell_item(Vector3i(pos.x, 0, pos.y)) == 0:
							if randi_range(1, 3) == 2:
								set_cell_item(Vector3i(pos.x, 0, pos.y), 2, 0)
				else:
					if IsValid(pos.x, pos.y) and get_cell_item(Vector3i(pos.x, 0, pos.y)) == 1 :
						set_cell_item(Vector3i(pos.x, 0, pos.y), 2, 0)
...
func PathTowards(start, goal, i):
	...
	set_cell_item(Vector3i(start.x, 0, start.y), 0, 0)
	set_cell_item(Vector3i(start.x, 0, start.y) + + Vector3i(1, 0, 0), 0, 0)
	set_cell_item(Vector3i(start.x, 0, start.y) + + Vector3i(-1, 0, 0), 0, 0)
	set_cell_item(Vector3i(start.x, 0, start.y) + + Vector3i(0, 0, 1), 0, 0)
...
func SpawnCoins():
	for i in Variables.roguelevelsid * 10:
		...
		if get_cell_item(Vector3i(x, 0, y)) != 1:
			
			var local = self.map_to_local(Vector3i(x, 0, y))
			var local2 = self.to_global(local)
			
			Variables.CoinManager.SpawnCoin(local2)

func SpawnDoor():
	
	var pos = Rooms[5]
	var local = self.map_to_local(Vector3i(pos.x, 0, pos.y))
	var local2 = self.to_global(local)
